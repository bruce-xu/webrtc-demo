  ...
  methods: {
    canSupportWebRTC () {
      if (typeof navigator.mediaDevices !== 'object') {
        ElMessage.error('No navigator.mediaDevices');
        return false;
      }
      if (typeof navigator.mediaDevices.enumerateDevices !== 'function') {
        ElMessage.error('No navigator.mediaDevices.enumerateDevices');
        return false;
      }
      if (typeof navigator.mediaDevices.getUserMedia !== 'function') {
        ElMessage.error('No navigator.mediaDevices.getUserMedia');
        return false;
      }
    ...
    initSocketEvents () {
      ...
      socket.on('checkRoomSuccess', (exsitRoomUsers) => {
        ...
        if (exsitRoomUsers && exsitRoomUsers.length > 1) {
          ElMessage.info('当前房间人数已满~请换个房间id');
        } else {
      ...
      socket.on('joinRoomSuccess', (roomUsers) => {
        ...
        ElMessage.success(`${otherUser.userName}加入了房间`);
      ...
      socket.on('userLeave', (roomUsers) => {
        ...
        this.roomUsers.forEach(item => {
          if (serverSockIdArr.indexOf(item.sockId) === -1) {
            ElMessage.info(`${item.userName}离开了房间`);
    ...
    submitForm () {
      if (!this.sockId) {
        ElMessage.error('socket未连接成功,请刷新再尝试!');
